...
begin
  ...
  cairo_set_source_rgb(ctx, 0.0, 0.0, 0.2);
  //cairo_paint(ctx);
  ...
  *)
  ...
  for i := 1 to 4 do
  begin
    cairo_arc(ctx, X1, Y1, R8, A3, A4);{ cairo_stroke(ctx);} cairo_fill(ctx);
    cairo_arc(ctx, -X1, Y1, R8, PI + A3, PI + A4);{ cairo_stroke(ctx);} cairo_fill(ctx);
  ...
  for i := 1 to 4 do
  begin
    cairo_arc(ctx, X2, Y2, R5, 0, 2 * PI);{ cairo_stroke(ctx);} cairo_fill(ctx);
    cairo_arc(ctx, -X2, Y2, R5, 0, 2 * PI);{ cairo_stroke(ctx);} cairo_fill(ctx);
    cairo_rotate(ctx, PI / 2);
  end;
  ...
  begin
    ...
    cairo_set_source_rgba(ctx, 0.00, 0.00, 0.00, 0.00);
    cairo_paint(ctx);
    cairo_restore(ctx);
