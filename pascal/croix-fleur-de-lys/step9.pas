...
begin
  ...
  for i := 1 to 4 do
  begin
    cairo_arc(ctx,  X1, Y1, R8,      A3,      A4); cairo_fill(ctx);
    cairo_arc(ctx, -X1, Y1, R8, PI + A3, PI + A4); cairo_fill(ctx);
  ...
  for i := 1 to 4 do
  begin
    cairo_arc(ctx,  X2, Y2, R5, A6, PI - A5);
    cairo_arc(ctx, -X2, Y2, R5, A5, PI - A6);
    cairo_rotate(ctx, PI / 2);
  end;
  cairo_fill(ctx);
  ...
  for i := 1 to 4 do
  begin
    cairo_arc(ctx, 0.0, R1, R5, 0, 2 * PI);
