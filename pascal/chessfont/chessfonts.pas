...
const
  ...
  EBChars = ' phbrqk+PHBRQK ojntwl+OJNTWL12345678'; // Eric Bentzen
...
function PositionToHtml(aPosition: TPiecePlacement; aChars: string; const aFontFamily, aColor, aFontSize: string): string;
...
begin
  ...
  result := Format('<p style="font-family:%s;color:%s;font-size:%s;">'#10, [aFontFamily, aColor, aFontSize]);
  for y := 9 downto 0 do
  begin
    for x := 0 to 9 do
      result := result + a[x, y];
