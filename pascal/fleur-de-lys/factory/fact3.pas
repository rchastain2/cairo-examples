...
begin
  fa := scale * StrToFloat(a);
  fb := scale * StrToFloat(b);
  fc := scale * StrToFloat(c); if rel then fc := fc - fa;
  fd := scale * StrToFloat(d); if rel then fd := fd - fb;
  fe := scale * StrToFloat(e); if rel then fe := fe - (fa + fc);
  ff := scale * StrToFloat(f); if rel then ff := ff - (fb + fd);
  WriteLn(Format('cairo%s_line_to(context, %f, %f);', [relstr[rel], fa, fb]));
  WriteLn(Format('cairo%s_line_to(context, %f, %f);', [relstr[rel], fc, fd]));
  WriteLn(Format('cairo%s_line_to(context, %f, %f);', [relstr[rel], fe, ff]));
end;
