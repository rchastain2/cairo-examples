...
begin
  ...
  cairo_set_source_surface(context, surf2, 0, 0);
  cairo_paint(context);
  ...
  cairo_arc(context, X, Y, 1 / 150, 0, 2 * PI);
  cairo_fill(context);
  ...
  cairo_set_source_rgb(context, 0, 0, 0);
  cairo_arc(context, xx, yy, 1 / 150, 0, 2 * PI);
  cairo_arc(context, xx1, yy1, 1 / 150, 0, 2 * PI);
  cairo_arc(context, xx2, yy2, 1 / 150, 0, 2 * PI);
  cairo_fill(context);
...
begin
  fangle := 0;
  surf2 := cairo_image_surface_create_from_png('image.png');
end;
