...
procedure CairoDraw(s1: PSDL_Surface);
var
...
begin
  ...
  WriteLn(Format('s1^.w=%d s1^.h=%d s1^.pitch=%d', [s1^.w, s1^.h, s1^.pitch]));
  WriteLn(Format('s1^.format^.format=%s', [SDL_GetPixelFormatName(s1^.format^.format)]));
  
  f1 := s1^.format;
  fe := f1^.format;
  
  WriteLn(Format('f1^.format=%s', [SDL_GetPixelFormatName(fe)]));
