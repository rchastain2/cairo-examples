...
for n as integer=1 to numsegments
  ...
  with s(n)
    ...
    .rd=FloatRange(0,1):.gr=FloatRange(0,1):.bl=FloatRange(0,1):.alph=FloatRange(.75,1)
    do
    i=IntRange(-1,1)
    loop until i<>0
...
dim as string v=__fb_version__
dim as string bd=__fb_build_date__

do
  ScreenLock
  Cls
  SetBackgroundColour((c,1,1,1,1))
  CPrint6(c,.01*xres,.1*yres,"FPS " & FrameCounter,50,(c,0,0,1,1))
  CPrint6(c,.01*xres,.15*yres,"FBC VERSION " & v,20,(c,1,0,0,1))
  CPrint6(c,.01*xres,.25*yres,"Build date " & bd,30,(c,0,0,0,1))
